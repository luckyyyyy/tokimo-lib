  ...
  docker run --rm \
    ...
    bash -eo pipefail -c '
      ...
      if [[ ! -f "$UDFREAD_PREFIX/lib/libudfread.a" ]]; then
        ...
        echo '' | base64 -d | sed "s/CCPLACEHOLDER/'${CC}'/;s/CXXPLACEHOLDER/'${CXX}'/;s/ARPLACEHOLDER/'${AR}'/;s/RANLIBPLACEHOLDER/'${RANLIB}'/;s/NMPLACEHOLDER/'${NM}'/" > "$CROSS_FILE"
